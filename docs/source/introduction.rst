...
It is designed to run using only the python standard library, and the only requirements are a working python (2.6 or above) installation and a working curses library.  Oscscreen will therefore work on almost all common platforms, and even in the Cygwin environment on Windows.
...
This bug is fixed in Python 3.4.1, and I was not alerted to it until 3.4.1 was already out.  I do not propose to release a workaround within oscscreen because I think the number of people who are stuck using Python 3.4.0 is very small.  If this causes a problem for you, please get in touch.
